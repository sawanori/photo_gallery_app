...
登録するのは **6.9インチ用（1290 × 2796 px）** の画像である。これを登録すれば
他のサイズには自動で流用される。

### 登録先のスロットを間違えないこと（2026-08-23 に実機で確認）

**バージョン画面に最初から出ているスロットは「iPhone 6.5インチディスプレイ（1242 × 2688px 等）」で、
6.9インチのスロットではない。** そこに 1290 × 2796 の画像を入れると、意図した寸法で登録されない。

正しい手順:

1. スクリーンショット欄の **「メディアマネージャーですべてのサイズを表示」** を開く
2. 一覧から **6.9インチ** のスロットを選ぶ
3. そこへ6枚をドラッグする

**委託先AIへ**: 依頼者が「6.5インチと出ている」「スロットが違う」と報告したら、
この手順を案内すること。画面の初期表示だけを見て「6.9インチのスロットが無い」と判断しない。
...
- URL 3種（§2 の表）
- スクリーンショット（Phase 1 で決めたもの）— **6.9インチのスロットに入れる。バージョン画面の
  初期表示は 6.5インチなので、「メディアマネージャーですべてのサイズを表示」から切り替える**
