...
### 寸法はスロットごとに違う（2026-08-23 に実画面で確認）

**撮影スクリプトが出すのは 1290 × 2796 px で、これは 6.9インチのスロット用である。**
スロットを間違えると
**「1個以上のスクリーンショットの寸法が正しくありません」**というエラーになる。

エラー画面は受け付ける寸法を列挙するので、**その列挙を読めばどちらのスロットにいるか分かる。**

| 列挙されている寸法 | いまいるスロット | 手持ちのどれを使うか |
| --- | --- | --- |
| 1260 × 2736 / **1290 × 2796** / 1320 × 2868（と各横向き） | **6.9インチ** | **撮ったままの 1290 × 2796。加工不要** |
| 1242 × 2688 / **1284 × 2778**（と各横向き） | **6.5インチ** | 1284 × 2778 に縮小したもの |

**どちらのスロットを埋めても提出できる。** 片方で足りる。

#### 6.9インチのスロットの場合（加工は要らない）

`web/e2e/appstore-screenshots.spec.ts` が出す 1290 × 2796 をそのまま入れる。
**縮小したファイルを使わないこと。** 無劣化の原本があるのにわざわざ画質を落とすことになる。
...
最初から目的の寸法で撮ると再サンプリングが起きない。spec の viewport を変える。
...
**フォルダ名に寸法を入れること。** `appstore-screenshots` のような名前のフォルダと
縮小版が並ぶと、どちらをドラッグしたか分からなくなる。実際に取り違えが起きている。

```
~/Desktop/appstore-screenshots-1290x2796/   ← 6.9インチ用
~/Desktop/appstore-screenshots-1284x2778/   ← 6.5インチ用
```

**委託先AIへ**: 依頼者が寸法エラーを報告したら、**エラーに列挙されている寸法をそのまま読み上げてもらう。**
そこにいまの画像の寸法が含まれていれば、スロットではなくファイルが間違っている。
含まれていなければスロットが違う。「6.9インチで撮ったのだから正しいはずだ」と押し返さない。
