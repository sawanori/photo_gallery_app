...
- [x] デモ用の招待の閲覧できる日数を 90 日にした（11月20日まで）
- [ ] `web` を本番にデプロイし、`https://gallery.non-turn.com/privacy` が開くことを確認する
- [ ] **デモ用のプロジェクトと招待を作る。実在のお客様の写真は使わない**
- [ ] **デモ招待の有効期限を十分先にする。** 審査は数日かかることがあり、
      期限切れの招待でレビュアーが「開けない」状態になると Guideline 2.1 で却下される
- [x] スクリーンショットを撮る（§7）— **人物の許諾だけ確認してください**
- [ ] App Store Connect でアプリレコードを作り、§1〜§11 を登録する
- [ ] `eas build --profile production --platform ios` で提出用ビルドを作る
