...
interface LineImageShareButtonProps {
  image: ImageType;
  size?: 'sm' | 'md';
}

export default function LineImageShareButton({ image, size = 'md' }: LineImageShareButtonProps) {
  const handleClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    const text = image.title
      ? `${image.title}\n${image.url}`
      : image.url;
    const lineUrl = `https://social-plugins.line.me/lineit/share?url=${encodeURIComponent(image.url)}&text=${encodeURIComponent(text)}`;
